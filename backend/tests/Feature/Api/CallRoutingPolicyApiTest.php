...
class CallRoutingPolicyApiTest extends TestCase
{
    ...
    {
        CallRoutingPolicy::factory()->count(3)->create(['organization_id' => $this->organization->id]);

        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/v1/organizations/{$this->organization->id}/call-routing-policies");
    ...
    {
        $response = $this->actingAs($this->user, 'sanctum')
            ->postJson("/api/v1/organizations/{$this->organization->id}/call-routing-policies", [
    ...
    {
        $policy = CallRoutingPolicy::factory()->create(['organization_id' => $this->organization->id]);

        $response = $this->actingAs($this->user, 'sanctum')
            ->getJson("/api/v1/organizations/{$this->organization->id}/call-routing-policies/{$policy->id}");
    ...
    public function test_can_update_a_call_routing_policy(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->putJson("/api/v1/organizations/{$this->organization->id}/call-routing-policies/{$policy->id}", [
                'name' => 'Updated Policy',
                'priority' => 50,
            ]);
    ...
    public function test_can_delete_a_call_routing_policy(): void
    {
        ...
        $response = $this->actingAs($this->user, 'sanctum')
            ->deleteJson("/api/v1/organizations/{$this->organization->id}/call-routing-policies/{$policy->id}");
    ...
    {
        CallRoutingPolicy::factory()->create(['organization_id' => $this->organization->id, 'priority' => 30]);
        CallRoutingPolicy::factory()->create(['organization_id' => $this->organization->id, 'priority' => 10]);
        CallRoutingPolicy::factory()->create(['organization_id' => $this->organization->id, 'priority' => 20]);
    ...
    public function test_cannot_access_another_organizations_policy(): void
    {
        $otherOrganization = Organization::factory()->create();
        $policy = CallRoutingPolicy::factory()->create(['organization_id' => $otherOrganization->id]);
