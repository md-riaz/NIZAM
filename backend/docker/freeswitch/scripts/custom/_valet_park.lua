-- _valet_park.lua
-- Auto-select an open valet parking orbit and park current call.
...
if not destination_number then
    freeswitch.consoleLog('ERR', string.format('[_valet_park] no open parking orbit in %s@%s (%d-%d)\n', lot, context, orbit_start, orbit_end))
    session:execute('respond', '486')
    return
end

freeswitch.consoleLog('NOTICE', string.format('[_valet_park] %s@%s parked in *%d via %s\n', caller_id_number, context, destination_number, orbit_entry_extension))
