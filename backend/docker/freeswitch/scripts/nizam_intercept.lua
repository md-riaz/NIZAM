-- nizam_intercept.lua
-- Directed call pickup helper based on FusionPBX intercept.lua behavior.
...
local function make_proxy_call(destination, call_hostname)
    ...
    freeswitch.consoleLog('INFO', string.format('[nizam_intercept] proxying pickup %s to %s\n', destination, call_hostname))
...
if not extension or extension == '' then
    freeswitch.consoleLog('WARNING', '[nizam_intercept] missing extension argument\n')
    return
end
...
if not selected_uuid or selected_uuid == '' then
    freeswitch.consoleLog('INFO', string.format('[nizam_intercept] no ringing call found for %s\n', extension))
    return
end
