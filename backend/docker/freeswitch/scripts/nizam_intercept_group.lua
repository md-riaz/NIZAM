-- nizam_intercept_group.lua
-- Group pickup helper. Uses tenant ring group membership as the pickup cohort.
-- Arguments: [direction]
...
local function make_proxy_call(destination, call_hostname)
    ...
    freeswitch.consoleLog('INFO', string.format('[nizam_intercept_group] proxying pickup %s to %s\n', destination, call_hostname))
...
if caller_extension == '' then
    freeswitch.consoleLog('WARNING', '[nizam_intercept_group] missing caller extension\n')
    return
end

local dbh = freeswitch.Dbh('odbc://nizam')
if not dbh or not dbh:connected() then
    freeswitch.consoleLog('ERR', '[nizam_intercept_group] unable to connect to database via ODBC DSN nizam\n')
    return
end
...
local membershipQuery = string.format(
    "SELECT members FROM ring_groups rg INNER JOIN tenants t ON t.id = rg.tenant_id WHERE t.domain = '%s' AND rg.is_active = 1",
    escapedDomain
)
...
if not hasPeers then
    freeswitch.consoleLog('INFO', string.format('[nizam_intercept_group] no pickup peers found for %s\n', caller_extension))
    return
end
...
if not selected_uuid or selected_uuid == '' then
    freeswitch.consoleLog('INFO', string.format('[nizam_intercept_group] no ringing group call found for %s\n', caller_extension))
    return
end
